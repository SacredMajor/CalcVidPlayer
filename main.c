...
static bool load_header_index(void) {
    /* Read LBA 0 → header */
    msd_transfer_t xfer;
    xfer.msd      = &g_global.msd;
    xfer.lba      = 0;
    xfer.count    = 1;
    xfer.buffer   = &g_header;
    xfer.callback = NULL;
    if (msd_Read(&xfer) != MSD_SUCCESS) return false;
    ...
    /* Read LBA 1..7 → index entries
     * Each entry = 64 bytes; 8 entries per sector.
     * We read all index sectors (up to 2) into a temp buffer.        */
